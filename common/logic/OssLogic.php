...
class OssLogic extends Instance
{
     ...
     * @return string
     */
    public function deleteOssFile($url)
    {
        $cdnUrl = '';
        if ($this->ossClient instanceof OssClient) {
            try {
                ...
                $cdnHost = "http://{$cdnDomain}/";
                $strSaveName = str_replace($cdnHost, '', $url);
                $this->ossClient->deleteObject($imgBucket,$strSaveName,  $options);
